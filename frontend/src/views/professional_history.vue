...
export default {
  ...
  mounted() {
    ...
    if (!token || (role !== "Professional" && role !== "Admin")) {
      this.$router.push('/');
    }
    this.fetchProfessionalHistory();
  },
  computed: {
    navComponent() {
  ...
  methods: {
    async create_csv() {
      ...
      const res = await fetch(`http://localhost:5000/create-csv/professional/ServiceRequest/${this.$route.params.id}`, {
        method: 'GET',
        headers: {
    ...
    async fetchProfessionalHistory() {
      try {
        const response = await fetch('http://localhost:5000/professional_history', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
