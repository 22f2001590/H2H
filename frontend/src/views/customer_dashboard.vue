                        ...
                        <div class="d-flex w-100 justify-content-between align-items-center">
                          <div class="mt-3 text-center">
                            ...
                            <p class="mb-0 text-muted">
                              <strong>Date:</strong> {{ formatDate(lr.needed_at) }} IST
                            </p>
                            <p class="mb-0 text-muted">
...
export default {
  ...
  data() {
    return {
      selectedRating: "",
      customerFeedback: "",
      user: { isActive: true }, // Example user data
      services: [], // Example services data
      liveRequests: [], // Example live requests
      searchTerm: "",
  ...
  methods: {
    ...
    async fetchLiveRequests() {
      try {
        ...
        const response = await fetch('http://localhost:5000/live_requests', {
          method: 'POST',  // POST method since you are sending data in the body
          headers: {
            'Content-Type': 'application/json'  // Ensure the body is sent as JSON
          },
          body: JSON.stringify({
            user_id: JSON.parse(localStorage.getItem("user")).id  // Add the user ID to the request body
          })
        });
        if (!response.ok) {
    ...
    async payServiceRequest(lr_id, lr_service_id) {
      try {
        ...
        // alert('An error occurred while paying for the service.');
      }
    },
    searchServices() {
    ...
    bookService(serviceId) {
      ...
      fetch('http://localhost:5000/book_now', {
        ...
        body: JSON.stringify(bookingData)  // Send the data as JSON
      })
        .then(response => response.json())  // Parse the JSON response
        .then(data => {
          console.log("Booking successful:", data);  // Log the success response
          // refresh the current page
          this.$router.go();
    ...
    async cancelRequest(requestId) {
      try {
        const response = await fetch(`http://localhost:5000/cancel_request/${requestId}`, {
          method: 'DELETE',  // Assuming the backend uses DELETE to remove the request
          headers: {
            'Content-Type': 'application/json',
          },
        });
    ...
    formatTimeForInput(dateString) {
      const timePart = dateString.split(' ')[4]; // Extract '15:50:00'
